module Admin
  ...
  class EventApprovalsController < BaseController
    def create
      ...
      return redirect_to admin_event_path(event), alert: "Solo se puede aprobar un evento enviado." unless event.submitted?

      result = Events::Approve.new(event).call
      if result.ok
        redirect_to admin_event_path(event), notice: "Evento aprobado, evento en Luma creado y correo enviado."
      else
