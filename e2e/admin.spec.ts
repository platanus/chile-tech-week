...
test('an admin moderates a submission', async ({ page }) => {
  ...
  execSync(
    `bin/rails runner 'Event.create!(edition: Edition::YEAR, title: ${JSON.stringify(title)}, description: "Un evento de prueba.", ` +
  ...
  const row = page.locator('a[href^="/admin/events/"]').filter({ hasText: title });
  ...
  await page.goto(`/admin/events?status=rejected&search=${encodeURIComponent(title)}`);
  await expect(page.locator('a[href^="/admin/events/"]').filter({ hasText: title })).toHaveCount(1);
});
...
test('an event waiting for its Luma edit shows its Luma details', async ({ page }) => {
  ...
  const id = execSync(
    `bin/rails runner 'print Event.create!(edition: Edition::YEAR, title: ${JSON.stringify(title)}, description: "Un evento de prueba.", ` +
      `company_name: "Platanus", company_website: "https://platan.us", company_logo_url: "/25/opengraph.png", ` +
      `author_name: "Ada Lovelace", author_email: "ada@platan.us", author_phone_number: "+56 9 8765 4321", ` +
      `starts_at: Time.zone.local(2026, 11, 17, 18), ends_at: Time.zone.local(2026, 11, 17, 20), ` +
...
test('search and the published list', async ({ page }) => {
  ...
  await expect(page.locator('a[href^="/admin/events/"]').first()).toContainText(/brunch/i);
